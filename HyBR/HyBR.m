function [x_out, output] = HyBR(A, b, P, options, mask)
%
% [x_out, output] = HyBR(A, b, P, options, mask)
%
...
%                       [non-negative scalar | GCV | {WGCV} | DP | UPRE | optimal]
...
%   mask : mask used for error computation and for computing optimal
...
% Chung and Ruthotto, modified 2015 for the QSM problem
...
if (strcmpi(regpar,'dp') || strcmpi(regpar,'upre')) && strcmpi(nLevel,'est')
    ...
    elseif ndims(b) == 2 % image
        [cA2,cH2,cV2,cD2] = dwt2(b,'db1');
        nLevel = median(abs(cD2(:)))/.67;
        options = HyBR_lsmrset(options, 'nLevel', nLevel);
    else
        error('Cannot estimate noise level of 3D image.')
    end
end
...
B = []; V = []; GCV = []; Omega= []; x_out = []; Alpha = []; alpha = 1;
    ...
    if i >= 2 %Begin Lanczos iterations
        ...
        switch insolve
            case {'tsvd', 'tikhonov'}
                ...
                [f, alpha] = feval(solverhandle, Ub, diag(Sb), Vb, vector, options, B, beta, V, m, mask, 1, alpha);
        ...
        if outputparams
            if ~notrue
                ...
                output.Enrm(i,1) = norm(mask.*x(:)-x_true(:))/nrmtrue;
            end
            output.Rnrm(i,1) = normr;
            output.Xnrm(i,1) = norm(mask.*x(:));
%             output.Xall(:,i) = mask.*x(:);
        end
